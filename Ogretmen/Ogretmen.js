...
export const Ogretmen = () => {
        ...
        const handleStartYoklamaForCourse = (e) => {//yoklama başlat
            e.preventDefault();
            axios.post('http://localhost:8080/api/yoklama/yoklamaDurumu', {
                ...
                setToastMessage("Seçtiğiniz dersin yoklaması başlatıldı.");
                setShowToast(true);
                setselectedDevamsizlik("");
                setselectedIzin("");
            })
                .catch(err => {
                    setToastMessage("Yoklama başlatılamadı!");
                    setShowToast(true);
                    console.log(err);
                });
        }

        const handleFinishYoklamaForCourse = (e) => {//yoklama başlat
        e.preventDefault();
        axios.post('http://localhost:8080/api/yoklama/yoklamaDurumu', {
            dersKodu: selectedCourse,
            yoklamaDurumu:false
            ...
            setToastMessage("Yoklama sonlandı.");
            setShowToast(true);
            setselectedDevamsizlik("");
            setselectedIzin("");
        })
        ...
        return (
            <div>
                ...
                <Button onClick={handleStartYoklamaForCourse}>Yoklama Başlat</Button>
                ...
                <Button variant="danger" onClick={handleFinishYoklamaForCourse}>Yoklama Sonlandır</Button>
                <Toast onClose={() => setShowToast(false)} show={showToast} autohide delay={2000}
                       style={{background: "green", position: 'fixed', top: 0, right: 0}}>
                    <Toast.Body>{toastMessage}</Toast.Body>
                </Toast>
